...
```{r splan}
##Load sample plan                         
sp <- read.csv(splan, header=FALSE)
fids <- gsub("[._]*R*[12]*.fastq(.gz)*", "", basename(as.character(sp[,3])), perl=TRUE)
bioids <- as.character(sp[,2])
names(bioids) <- fids
```
...
## Force sampleID as character
statsFile$sampleID <- as.character(statsFile$sampleID)
...
gg <- ggplot(dataPlot, aes(x=as.character(sample), y=n_read, fill=type, text=percent)) + geom_bar(stat="identity") + scale_fill_manual(name = "", values=rev(c("orange", "darkorange4", "red3", "gray"))) + 
   theme_minimal() +
...
if (length(preseq.in) > 0){
   ...
   names(preseq.info) <- as.character(sp[match(basename(sub("preseq","",dirname(preseq.in))), sp[,1]),2])
...
counts <- read.csv(file.path(input_path,"report/tablecounts_raw.csv"), row.names=1, check.names=FALSE)
...
colnames =c('Sample', 'Biosample', 'Number of mapped reads', 'Number of exonic reads', 'Percent of exonic reads', 'Number of intronic reads', 'Percent of intronic reads', 
'Number of intergenic reads', 'Percent of intergenic reads'), rownames = FALSE, filter = 'top', caption = 'Reads annotation', options = list(columnDefs = list(list(className = 'dt-center', targets = 2:5))))
...
gg <- ggplot(dataPlot, aes(x=as.character(sample), y=n_read, fill=type, text = paste("--Sample: ",sample,"<br>","Percent: ",percent,"%<br>","n_read: ",n_read))) + 
...
counts.tpm <- read.csv(file.path(input_path,"report/tablecounts_tpm.csv"), row.names=1, check.names=FALSE)
...
if (nrow(d2p) > (n_items + 1)){
   d2p <- rbind(d2p[1:n_items,], others=colSums(d2p[(n_items+1):nrow(d2p), , drop=FALSE]))
}
...
datatable(format(t(d2p), big.mark=",",scientific=FALSE), colnames = rownames(d2p),
 rownames = TRUE, filter = 'top', caption = 'Gene annotation', options=list(columnDefs = list(list(className = 'dt-center', targets=1:nrow(d2p)))))
...
if (nrow(d2p) > 1){
   ...
   d2p <- d2p[setdiff(rownames(d2p), "Total"),, drop=FALSE]
   gg <- ggplot(melt(d2p), aes(x=as.character(Var2), y=value, fill=Var1)) + 
