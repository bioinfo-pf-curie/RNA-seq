...
#inputFiles <-"MAP567_WTS_bcftools.tsv"
#outputFile<-"test_matrix.tsv"
#polym<-"/data/tmp/tgutman/EUCANCAN/test_clustering/test_clustering/44polyms.bed"
#minDP<-10

## load BED with polym
bed <- read.table(polym,stringsAsFactors = F)[,c(1,3:4),]
bed <- cbind(bed,data.frame(Reduce(function(x,y) rbind(x,y), strsplit(gsub(">","_",as.character(bed[,"V4"])),"_"),accumulate=F)))
colnames(bed) <- c("chr","pos","name","gene","rs","ref_polym","alt_polym")

## load tsv file from SnpSift with AD & DP:

var_tsv=read.table(inputFiles, header=TRUE, stringsAsFactors = F)
...
#match(polym_table$alt_polym[4], unlist(strsplit(polym_table$alt[4],",")))
