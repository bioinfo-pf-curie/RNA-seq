...
do
	...
	if ls picard/${sample}Aligned*markDups_metrics.txt  1>/dev/null 2>&1; then 
	    if [ $is_pe == "1" ]; then
  		n_dup=$(grep -a2 "## METRICS" picard/${sample}Aligned*markDups_metrics.txt | tail -1 | awk -F"\t" '{print $7}')
	    else
		n_dup=$(grep -a2 "## METRICS" picard/${sample}Aligned*markDups_metrics.txt | tail -1 | awk -F"\t" '{print $6}')
	    fi
